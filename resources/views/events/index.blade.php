...
>
    @php
        ...
        $sectionTitle = match (true) {
            (bool) $search => __('Search results'),
            (bool) $category => \App\Models\Event::CATEGORIES[$category] ?? __('Events'),
            $when === 'today' => __('Today'),
            $when === 'weekend' => __('This weekend'),
            default => __('Must-see events'),
        };
    @endphp
    ...
    <div class="min-h-screen bg-white dark:bg-[#0F0F0F]"
        ...
        <section id="hero-search" class="relative overflow-hidden bg-white dark:bg-[#0F0F0F] border-b border-charcoal/[0.05] dark:border-white/10">
            ...
            <div class="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20 text-center">
                ...
                <p class="mt-4 text-base sm:text-lg text-frost max-w-2xl mx-auto leading-relaxed">
                    {{ __('Discover and book the best events around you.') }}
                    <br>
                    {{ __('Concerts, conferences, festivals, shows, and much more.') }}
                </p>
                ...
                <form method="GET" action="{{ route('events.index') }}" class="mt-8 max-w-xl mx-auto w-full">
                    ...
                    @endif
                    <label for="catalog-search" class="sr-only">{{ __('Search') }}</label>
                    <div class="flex items-center gap-2 rounded-full border border-charcoal/[0.1] dark:border-white/10 bg-white dark:bg-[#1A1A1A] shadow-sm pl-5 pr-1.5 py-1.5 focus-within:ring-2 focus-within:ring-coral/20">
                        <x-icon name="magnifying-glass" class="w-5 h-5 text-frost shrink-0" />
                        <input id="catalog-search" type="text" name="search"
                            placeholder="{{ __('Concert, conference, venue…') }}"
                            class="flex-1 min-w-0 bg-transparent border-0 focus:ring-0 text-sm sm:text-base text-charcoal dark:text-[#FAFAFA] placeholder:text-frost py-2.5 sm:py-3"
                            value="{{ $search ?? '' }}">
        ...
        <section id="categories" class="bg-cream/50 dark:bg-[#141414] py-12 sm:py-16">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="text-center mb-8 sm:mb-10">
                    ...
                    <p class="mt-2 text-sm text-frost">{{ __('Find the event that suits you') }}</p>
                </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 sm:gap-5">
                    ...
                    <a href="{{ route('events.index') }}#events"
                       class="group flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-charcoal/15 dark:border-white/15 bg-white dark:bg-[#1A1A1A] p-5 sm:p-6 text-center no-underline transition hover:border-coral/40 hover:bg-coral-muted/30">
                        <span class="flex h-12 w-12 items-center justify-center rounded-xl bg-charcoal/[0.04] dark:bg-white/5 text-charcoal dark:text-[#FAFAFA] group-hover:bg-coral group-hover:text-white transition">
                            <x-icon name="plus" class="h-6 w-6" />
                        </span>
                        <span class="text-sm font-semibold text-charcoal dark:text-[#FAFAFA]">{{ __('See more') }}</span>
                        <span class="text-xs text-frost">{{ __('Full catalog') }}</span>
                    </a>
                </div>
            </div>
        </section>
        ...
        <section id="events" class="bg-white dark:bg-[#0F0F0F] py-12 sm:py-14">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex flex-wrap items-end justify-between gap-3 mb-10">
                    <div>
                        <h2 class="text-xl sm:text-2xl font-bold text-charcoal dark:text-[#FAFAFA]">{{ $sectionTitle }}</h2>
                        @if ($search)
                            <p class="mt-1 text-sm text-frost">« {{ $search }} »</p>
                        @elseif (! $category)
                            <p class="mt-1 text-sm text-frost">{{ __('The best experiences near you') }}</p>
                        @endif
                        @if ($search || $category)
                    ...
                    <p class="text-sm text-frost">
                        <span x-text="total" class="font-semibold text-charcoal dark:text-[#FAFAFA]">{{ $events->total() }}</span>
                        {{ __('events listed') }}
                    </p>
                </div>
        ...
        {{-- Avantages --}}
        <section class="border-t border-charcoal/[0.06] dark:border-white/10 bg-cream/40 dark:bg-[#141414] py-12 sm:py-14">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    <div class="text-center sm:text-left">
                        ...
                        <p class="font-semibold text-charcoal dark:text-[#FAFAFA]">{{ __('Fast booking') }}</p>
                        <p class="mt-1 text-sm text-frost leading-relaxed">{{ __('Tickets confirmed in seconds.') }}</p>
                    </div>
                    <div class="text-center sm:text-left">
                        <span class="inline-flex h-11 w-11 items-center justify-center rounded-xl bg-coral-muted text-coral mb-3">
                    ...
                    <div class="text-center sm:text-left">
                        ...
                        <p class="font-semibold text-charcoal dark:text-[#FAFAFA]">{{ __('Mobile ticket') }}</p>
                        <p class="mt-1 text-sm text-frost leading-relaxed">{{ __('QR code on your phone, ready to scan.') }}</p>
                    </div>
                    <div class="text-center sm:text-left">
                        <span class="inline-flex h-11 w-11 items-center justify-center rounded-xl bg-coral-muted text-coral mb-3">
                            <x-icon name="phone" class="w-5 h-5" />
                        </span>
                        <p class="font-semibold text-charcoal dark:text-[#FAFAFA]">{{ __('Support 24/7') }}</p>
                        <p class="mt-1 text-sm text-frost leading-relaxed">{{ __('A team available to help you.') }}</p>
                    </div>
                </div>
            </div>
        </section>
